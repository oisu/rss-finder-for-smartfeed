    ...
    return new Promise(function(resolve, reject) {
        ...
        parser = new htmlparser.Parser({
            onopentag: function(name, attr) {
                var _pushFeed = function(title, url) {
                    if (/((rss)|(xml)|(rdf)|(feeds?))\/?(\?(.*))?$/i.test(url)) {
                        feeds.push({
                            title: title,
                            url: url
                        });
                ...
                if (name === 'link' && (rssTypes.indexOf(attr.type) !== -1)) {
                    _pushFeed(attr.title, attr.href);
                }
                ...
                if (name === 'a' && /((feed)|(atom)|(rdf)|(rss))$/.test(attr.href)) {
                    _pushFeed(null, attr.href);
                }
